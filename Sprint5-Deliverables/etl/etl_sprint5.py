"""
Sprint 5 - Incremental ETL Pipeline with DGDB Integration
============================================================
Based on Sprint 4's etl_incremental.py, enhanced with:
  - Data Governance Database (DGDB) logging via SQLAlchemy ORM
  - Auto-creation of HotelDGDB database and tables on first run
  - ETL run tracking (start/end/status/counts)
  - Data quality validation rules executed after loading
  - Validation results logged to DGDB
...
Incremental Strategy:
  - Uses __ETLMetadata table to track last load state per source→target pair
...
Usage:
    python etl_sprint5.py              # Incremental (default)
    python etl_sprint5.py --full       # Full refresh
"""
...
# Add parent directory to path for dgdb/validation imports
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from dgdb.dgdb_client import DGDBClient
from validation.validation_rules import run_all_validations
...
def main():
    ...
    print("=" * 70)
    print(f"Sprint 5 ETL Pipeline -- {mode} -- with DGDB Integration")
    print("=" * 70)
    ...
    # -- Step 0: Initialize DGDB (auto-creates DB + tables if needed) --
    print("\n--- Initializing Data Governance Database (DGDB) ---")
    dgdb = DGDBClient(SQL_SERVER, SQL_USER, SQL_PASSWORD)
    run_id = dgdb.start_run(f"Sprint 5 ETL -- {mode}")

    # Track totals for DGDB logging
    total_processed = 0
    total_rejected = 0
    ...
    try:
        ...
        try:
            ...
            total_processed += rev_count

            checkin_count = load_fact_checkin(is_full, prop_map, cust_map)
            total_processed += checkin_count

            gift_count, gift_skipped = load_fact_gift_shop_sales(
                mongo_db, shops_raw, products_raw,
                shop_map, prod_gift_map, prop_map, is_full
            )
            total_processed += gift_count
            total_rejected += gift_skipped

            payroll_count = load_fact_payroll(is_full, prop_map, emp_map)
            total_processed += payroll_count

            amenity_count = load_fact_property_amenity(is_full, prop_map, amen_map)
            total_processed += amenity_count

            # -- Phase 4: Verify --
            verify_all()

            # -- Phase 5: Validation rules (logged to DGDB) --
            run_all_validations(
                dgdb, run_id, get_dw_conn, get_source_conn,
                MONGO_URI, MONGO_DATABASE
            )

            # -- Log successful completion to DGDB --
            ...
            dgdb.end_run(
                ...
                notes=(
                    f"fact_revenue: {rev_count:,} | fact_checkin: {checkin_count:,} | "
                    f"fact_gift_shop_sales: {gift_count:,} | fact_payroll: {payroll_count:,} | "
                    f"fact_property_amenity: {amenity_count:,}"
                )
            )
            ...
            print(f"  fact_property_amenity: {amenity_count:>12,} rows")
            print(f"  TOTAL processed:      {total_processed:>12,} rows")
            print(f"  TOTAL rejected:       {total_rejected:>12,} rows")
            print(f"{'=' * 70}")
